/**
 * Verify v12 (v11 + overnight veto) beats v11 on next-session OTC.
 * Run: npx tsx scripts/verify-v12-overnight-veto.ts
 */
...
async function main() {
  // Units
  {
    const g = computeOvernightGapPct(101, 100);
    if (g == null || Math.abs(g - 1) > 1e-9) throw new Error(`gap expected 1, got ${g}`);
    if (applyOvernightVeto(40, -0.5) !== 0) throw new Error("long vs gap-down should veto");
    if (applyOvernightVeto(-40, 0.5) !== 0) throw new Error("short vs gap-up should veto");
    if (applyOvernightVeto(40, 0.5) !== 40) throw new Error("long vs gap-up should keep");
    if (applyOvernightVeto(40, -0.1) !== 40) throw new Error("small fight should keep");
    console.log("ok: overnight veto unit checks");
  }

  if (!ANALOG_MODEL_SETTINGS.overnightVetoEnabled) {
    throw new Error("overnightVetoEnabled should be true");
  }
  if (!MODEL_VERSION.includes("v12")) {
    throw new Error(`expected v12 model, got ${MODEL_VERSION}`);
  }
  ...
  function run(opts: { fade: boolean; overnightVeto: boolean }): Day[] {
    const days: Day[] = [];
    for (let ti = start; ti < st.length; ti++) {
      let s = knnScore(ti);
      if (opts.fade) s = applyFadeBigDay(s, st[ti].ctc);
      if (opts.overnightVeto) {
        s = applyOvernightVeto(s, st[ti].overnight, STRATEGY_RULES.scoreThreshold);
      }
  ...
  const v11Days = run({ fade: true, overnightVeto: false });
  const v12Days = run({ fade: true, overnightVeto: true });
  const v11 = summarize(v11Days);
  const v12 = summarize(v12Days);
  const y11 = yearStab(v11Days);
  const y12 = yearStab(v12Days);

  console.log("\n========== NEXT-SESSION OTC ==========");
  console.log(
    `v11 fade+Mon         hit=${v11.hit.toFixed(1)}% edge=${v11.edge.toFixed(3)} cash=${(v11.cash * 100).toFixed(0)}% dir=${v11.dir} fair=${v11.fair.toFixed(4)} yE+${y11.edgePos}/${y11.years}`,
  );
  console.log(
    `v12 +overnightVeto   hit=${v12.hit.toFixed(1)}% edge=${v12.edge.toFixed(3)} cash=${(v12.cash * 100).toFixed(0)}% dir=${v12.dir} fair=${v12.fair.toFixed(4)} yE+${y12.edgePos}/${y12.years}`,
  );

  const pass =
    v12.fair > v11.fair + 0.05 &&
    v12.edge > v11.edge &&
    v12.hit > v11.hit &&
    v12.dir >= 250 &&
    v12.cash < 0.85 &&
  ...
  console.log("\n========== VERDICT ==========");
  if (!pass) {
    console.log("FAIL: v12 does not clearly beat v11");
    process.exit(2);
  }
  console.log(`PASS: lock ${MODEL_VERSION}`);
  console.log(
    `  fair ${v11.fair.toFixed(4)} → ${v12.fair.toFixed(4)} | hit ${v11.hit.toFixed(1)}→${v12.hit.toFixed(1)} | edge ${v11.edge.toFixed(3)}→${v12.edge.toFixed(3)} | cash ${(v11.cash * 100).toFixed(0)}→${(v12.cash * 100).toFixed(0)}%`,
  );
  process.exit(0);
}
