/**
 * Verify v15 blend 0.55/0.45 beats v14 blend 0.4/0.6 on next-session OTC.
 * Run: npx tsx scripts/verify-v15-blend.ts
 */
...
async function main() {
  if (Math.abs(ANALOG_MODEL_SETTINGS.oneDayBlendWeight - 0.55) > 1e-9) {
    throw new Error(`expected w1=0.55, got ${ANALOG_MODEL_SETTINGS.oneDayBlendWeight}`);
  }
  if (Math.abs(ANALOG_MODEL_SETTINGS.threeDayBlendWeight - 0.45) > 1e-9) {
    throw new Error(`expected w3=0.45, got ${ANALOG_MODEL_SETTINGS.threeDayBlendWeight}`);
  }
  if (!MODEL_VERSION.includes("v15")) {
    throw new Error(`expected v15, got ${MODEL_VERSION}`);
  }
  console.log("ok: blend weights locked at 0.55/0.45");
  ...
  function knn(ti: number, w1: number, w3: number) {
    const today = st[ti];
  ...
  function run(w1: number, w3: number): Day[] {
    const days: Day[] = [];
    for (let ti = start; ti < st.length; ti++) {
      const { score: knnS, raws } = knn(ti, w1, w3);
      let s = applyLowNeighborVolFlat(knnS, raws);
  ...
  const d14 = run(0.4, 0.6);
  const d15 = run(0.55, 0.45);
  const v14 = summarize(d14);
  const v15 = summarize(d15);
  const y14 = yearStab(d14);
  const y15 = yearStab(d15);

  console.log("\n========== NEXT-SESSION OTC ==========");
  console.log(
    `v14 blend 0.4/0.6   hit=${v14.hit.toFixed(1)}% edge=${v14.edge.toFixed(3)} cash=${(v14.cash * 100).toFixed(0)}% dir=${v14.dir} fair=${v14.fair.toFixed(4)} yE+${y14.edgePos}/${y14.years}`,
  );
  console.log(
    `v15 blend 0.55/0.45 hit=${v15.hit.toFixed(1)}% edge=${v15.edge.toFixed(3)} cash=${(v15.cash * 100).toFixed(0)}% dir=${v15.dir} fair=${v15.fair.toFixed(4)} yE+${y15.edgePos}/${y15.years}`,
  );

  const pass =
    v15.fair > v14.fair + 0.04 &&
    v15.edge > v14.edge &&
    v15.dir >= 250 &&
    v15.cash < 0.88 &&
    y15.edgePos >= y14.edgePos;

  console.log("\n========== VERDICT ==========");
  if (!pass) {
    console.log("FAIL: v15 does not clearly beat v14");
    process.exit(2);
  }
  console.log(`PASS: lock ${MODEL_VERSION}`);
  console.log(
    `  fair ${v14.fair.toFixed(4)} → ${v15.fair.toFixed(4)} | hit ${v14.hit.toFixed(1)}→${v15.hit.toFixed(1)} | edge ${v14.edge.toFixed(3)}→${v15.edge.toFixed(3)} | yE+ ${y14.edgePos}→${y15.edgePos}`,
  );
  process.exit(0);
}
