/**
 * One-off ablation: OTC labels vs A/B-only vs baseline (fair product metrics).
 * Run: npx tsx scripts/ablate-v9-package.ts
 */
...
/** Pre-v9: C/D can still be directional. */
function buildSignalAllowCD(
  score: number,
  nret: number[],
  ndist: number[],
  rules: typeof STOCKS_STRATEGY_RULES,
): TradableSignal {
  const meanD = ndist.reduce((s, d) => s + d, 0) / ndist.length;
  const dq = distanceQualityFromMean(meanD);
  const scoreSign = score > 0 ? 1 : score < 0 ? -1 : 0;
  const agr =
    scoreSign === 0
      ? nret.filter((v) => Math.abs(v) < 0.15).length / nret.length
      : nret.filter((v) => v * scoreSign > 0).length / nret.length;
  const rel = reliabilityFromMetrics(meanD, agr, dq, rules);
  if (rel === "F") {
    return {
      position: "NO_TRADE",
      size: 0,
      reliability: rel,
      neighborAgreement: Number(agr.toFixed(4)),
      meanNeighborDistance: Number(meanD.toFixed(4)),
      distanceQuality: dq,
      noTrade: true,
      reason: "F",
    };
  ...
  const position = score > 0 ? "LONG" : "SHORT";
  const magnitude = Math.min(1, Math.abs(score) / 100);
  const size = Number(Math.min(1, magnitude * agr * dq).toFixed(4));
  return {
    position,
    size,
    reliability: rel,
...
async function main() {
  ...
  type Day = { score: number; signal: TradableSignal; otc: number; ctc: number };

  function run(
    features: string[],
    labelMode: "ctc" | "otc",
    mode: "naive" | "gates_cd" | "gates_ab",
  ): Day[] {
    ...
    for (let ti = start; ti < st.length; ti++) {
      ...
      const f1 = (p: P) => (labelMode === "otc" ? p.f1Otc : p.f1Ctc);
      const f3 = (p: P) => (labelMode === "otc" ? p.f3Otc : p.f3Ctc);
      const ranked = pool
        .filter((p) => f1(p) != null)
      ...
      } else if (mode === "gates_cd") {
        signal = buildSignalAllowCD(score, nret, ndist, STOCKS_STRATEGY_RULES);
      } else {
      ...
      days.push({ score, signal, otc: today.otc, ctc: today.ctc });
    }
    return days;
  }

  function evalLagged(days: Day[], horizon: "ctc" | "otc", useSignal: boolean) {
    let dir = 0,
      hits = 0,
      cash = 0;
    const longs: number[] = [];
    ...
    for (let i = 1; i < days.length; i++) {
      const p = days[i - 1];
      const s = days[i];
      const ret = horizon === "ctc" ? s.ctc : s.otc;
  ...
  const configs: Array<{
    ...
    { name: "BASE CTC + naive", features: allKeys, label: "ctc", mode: "naive", useSig: false },
    {
      name: "v8 CTC + gates C/D ok",
      features: knnKeys,
      label: "ctc",
      mode: "gates_cd",
      useSig: true,
  ...
  console.log("=== OTC EVAL (product horizon) ===");
  for (const c of configs) {
    const days = run(c.features, c.label, c.mode);
    const r = evalLagged(days, "otc", c.useSig);
    console.log(
      c.name.padEnd(28),
      "hit=" +
        r.hit.toFixed(1) +
        ...
        r.whenIn.toFixed(3) +
        " cash=" +
        (r.cashR * 100).toFixed(1) +
        "% dir=" +
        r.dir +
        " fair=" +
        r.fair.toFixed(4) +
        " AB_hit=" +
        (r.hiHit?.toFixed(1) ?? "n/a") +
        " n=" +
        r.hiN,
    );
  }
  ...
  for (const c of configs) {
    const days = run(c.features, c.label, c.mode);
    const r = evalLagged(days, "ctc", c.useSig);
    console.log(
      c.name.padEnd(28),
      "hit=" +
        r.hit.toFixed(1) +
