/**
 * Verify v11 (cosine + Mon skip + fade-big-day) beats v10 on next-session OTC.
 * Run: npx tsx scripts/verify-v11-fade-big-day.ts
 */
...
async function main() {
  // Unit: fade helper
  {
    const unchanged = applyFadeBigDay(40, 0.5);
    if (unchanged !== 40) throw new Error(`no fade expected, got ${unchanged}`);
    const faded = applyFadeBigDay(40, 2.0); // big up day → push down
    // 40*0.3 - 25 = 12-25 = -13
    if (faded !== -13) throw new Error(`fade up day expected -13, got ${faded}`);
    const fadedDn = applyFadeBigDay(-40, -2.0); // big down → push up
    // -40*0.3 - (-1)*25 = -12+25 = 13
    if (fadedDn !== 13) throw new Error(`fade down day expected 13, got ${fadedDn}`);
    console.log("ok: applyFadeBigDay unit checks");
  }

  if (!ANALOG_MODEL_SETTINGS.fadeBigDayEnabled) {
    throw new Error("fadeBigDayEnabled should be true for v11");
  }
  if (!MODEL_VERSION.includes("v11")) {
    throw new Error(`expected v11 model version, got ${MODEL_VERSION}`);
  }
  ...
  function run(fade: boolean): Day[] {
    const days: Day[] = [];
    for (let ti = start; ti < st.length; ti++) {
      let s = knnScore(ti);
      if (fade) s = applyFadeBigDay(s, st[ti].ctc);
  ...
  const v10 = summarize(run(false));
  const v11 = summarize(run(true));

  console.log("\n========== NEXT-SESSION OTC ==========");
  console.log(
    `v10 cosine+Mon     hit=${v10.hit.toFixed(1)}% edge=${v10.edge.toFixed(3)} cash=${(v10.cash * 100).toFixed(0)}% dir=${v10.dir} fair=${v10.fair.toFixed(4)}`,
  );
  console.log(
    `v11 +fadeBigDay    hit=${v11.hit.toFixed(1)}% edge=${v11.edge.toFixed(3)} cash=${(v11.cash * 100).toFixed(0)}% dir=${v11.dir} fair=${v11.fair.toFixed(4)}`,
  );

  const pass =
    v11.fair > v10.fair + 0.05 &&
    v11.edge > v10.edge &&
    v11.dir >= 400 &&
    v11.cash < 0.8;
  ...
  console.log("\n========== VERDICT ==========");
  if (!pass) {
    console.log("FAIL: v11 does not clearly beat v10");
    process.exit(2);
  }
  console.log(`PASS: lock ${MODEL_VERSION}`);
  console.log(
    `  fair ${v10.fair.toFixed(4)} → ${v11.fair.toFixed(4)} | hit ${v10.hit.toFixed(1)}→${v11.hit.toFixed(1)} | edge ${v10.edge.toFixed(3)}→${v11.edge.toFixed(3)}`,
  );
  process.exit(0);
}
