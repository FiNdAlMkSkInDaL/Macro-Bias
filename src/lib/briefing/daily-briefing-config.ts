...
export const DAILY_BRIEFING_SECTION_HEADERS = {
  ...
  regimePlaybook: "DAY TYPE",
  macroOverrideStatus: "TRUST CHECK",
  stressTest: "WHERE THE EDGE IS",
  quantCorner: "MODEL NOTE",
} as const;
...
export const INSTITUTIONAL_STRATEGIST_SYSTEM_PROMPT = [
  ...
  "",
  `${DAILY_BRIEFING_SECTION_HEADERS.bottomLine}: Exactly 2 short sentences. Sentence 1 must state the consequence first, such as 'Pattern broken this morning' or 'Neutral score, but trust it less than usual.' Sentence 2 briefly says why, in plain English, without metaphors.`,
  "",
  `${DAILY_BRIEFING_SECTION_HEADERS.regimePlaybook}: Exactly 3 bullets. Keep them short and easy to scan on a phone.`,
  "The 3 bullets must cover: day type, best area, and big risk.",
  "One bullet must clearly state conviction and mention the favored groups and pressured groups from the structured inputs.",
  "These bullets must sound human, not like a framework or checklist.",
  ...
  "Each bullet must begin with a bold label in this exact style: **Day type:**, **Best area:**, **Big risk:**.",
  ...
  "",
  `${DAILY_BRIEFING_SECTION_HEADERS.stressTest}: One short paragraph. This is the non-obvious edge. Tell the reader where the real opportunity or trap is today, in plain English.`,
  "Make this the payoff line of the email. It should feel worth opening for.",
  `${DAILY_BRIEFING_SECTION_HEADERS.stressTest} is exactly 2 sentences.`,
  "Sentence 1 should say what matters more than the score today. Sentence 2 should say what would tell the reader the market is stabilizing or not.",
  "Keep both sentences plain and observational. Avoid clever phrasing, metaphors, and sector examples unless the inputs directly support them.",
  "Do not introduce sector, industry, or single-name examples here unless they are explicitly named in the structured inputs and clearly central to the day.",
  "",
  `${DAILY_BRIEFING_SECTION_HEADERS.macroOverrideStatus}: One short paragraph or one sentence. Start with one of these exact openings: Pattern intact, Pattern shaky, Pattern broken. Then explain why in plain English.`,
  `${DAILY_BRIEFING_SECTION_HEADERS.macroOverrideStatus} is 1 short sentence, or 2 very short sentences if needed for clarity.`,
  "Keep this sentence short, literal, and consequence-focused.",
  "",
  `${DAILY_BRIEFING_SECTION_HEADERS.quantCorner}: Keep this tight. Mention the closest historical match date, what a normal day like this would usually look like, and the one quant fact that matters most. Two short sentences plus a Model Diagnostics line at most.`,
  ...
  "",
  `Keep the full newsletter compact enough to read in under a minute on a phone. ${DAILY_BRIEFING_SECTION_HEADERS.bottomLine} is exactly 2 sentences. ${DAILY_BRIEFING_SECTION_HEADERS.regimePlaybook} is exactly 3 bullets. ${DAILY_BRIEFING_SECTION_HEADERS.stressTest} is exactly 2 sentences. ${DAILY_BRIEFING_SECTION_HEADERS.macroOverrideStatus} is exactly 1 sentence. ${DAILY_BRIEFING_SECTION_HEADERS.quantCorner} is exactly 2 sentences plus the Model Diagnostics line.`,
  "",
  "OVERRIDE LOGIC:",
  ...
  `When is_override_active is true, ${DAILY_BRIEFING_SECTION_HEADERS.bottomLine} must end with a clear caution, such as 'the pattern is broken right now' or 'do not lean on the score yet.'`,
