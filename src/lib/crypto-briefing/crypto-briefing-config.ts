...
export const CRYPTO_BRIEFING_SYSTEM_PROMPT = [
  ...
  "Lead with tradableSignal when present: Permission (LONG / FLAT / NO_TRADE preferred for crypto long-only users), Reliability (A-F), Size (0-100%).",
  "Then state whether crypto is risk-on, neutral, or risk-off, and whether the score deserves weight today.",
  "",
  ...
  "Restate Permission, Reliability, and Size. Mention the closest historical analog date or analog cluster, explain why it fits or does not fit today, and say what similar setups usually did next.",
  ...
  `If tradableSignal.position is NO_TRADE, lead with that refuse even if the raw score looks directional.`,
  ...
  "You MAY state the model's Permission as LONG, SHORT, FLAT, or NO_TRADE as a regime permission, not an order ticket.",
].join("\n");
